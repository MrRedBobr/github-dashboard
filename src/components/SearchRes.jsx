		...
		<li className={style.item}>
			...
			<div>
				...
				<div>
					...
					<a>
						Updated on{' '}
						{props.repo.updated_at.split('T')[0].replace(/(-)/g, '.')}
					</a>
					<a className={style.url} href={props.repo.html_url}>
						☣
					</a>
				</div>
			</div>
...
export class SearchRes extends React.Component {
	...
	render() {
		return (
			<ul>
				{this.props.res.map((val, index) => (
					<Item repo={val} key={index} index={index} />
				))}
			</ul>
		);
	}
